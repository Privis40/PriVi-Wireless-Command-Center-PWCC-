...
"""
╔══════════════════════════════════════════════════════════════════╗
║       PriVi Wireless Command Center (PWCC) v6.0                  ║
...
def signal_sparkline(bssid: str) -> str:
    ...
    spark  = ""
    for v in values:
        idx    = min(int((v - mn) / rng * (len(bars) - 1)), len(bars) - 1)
        spark += bars[idx]
    return spark
...
def check_hidden_ssid(pkt):
    ...
    if pkt.haslayer(Dot11AssoReq) or pkt.haslayer(Dot11ReassoReq):
        ...
        if ap_mac in networks and len(networks[ap_mac]) >= 1 and networks[ap_mac][0] in ("Hidden", "", "\x00"):
            try:
...
def packet_callback(pkt, lab_ssid: str, lab_bssid: str):
    ...
    if pkt.haslayer(Dot11ProbeReq):
        ...
        if client_mac and is_real_client(client_mac):
            if client_mac not in client_info:
                client_info[client_mac] = {
                    "ssid": "Probing...", "bssid": "---",
                    "vendor": mac_vendors.get(client_mac, ""),
                    "last_seen": time.time()
                }
                resolve_vendor_bg(client_mac)
            else:
        ...
        if addr2 and addr3 and addr3 in networks and len(networks[addr3]) >= 1:
            if is_real_client(addr2) and addr2 != addr3:
                ...
                if addr2 not in client_info:
                    ...
                    log_event(f"[cyan]CLIENT: {addr2} on {net_ssid}[/cyan]")
                else:
                    client_info[addr2]["last_seen"] = time.time()

        if addr1 and addr3 and addr3 in networks and len(networks[addr3]) >= 1:
    ...
    if pkt.haslayer(EAPOL):
        ...
        ssid = (networks[bss][0] if bss in networks and len(networks[bss]) >= 1 else "Unknown")
...
def build_client_table() -> RichTable:
    ...
    1. CONNECTED — have a confirmed AP BSSID (from data/association frames)
    2. PROBING   — scanning for networks, not yet associated
    """
    t = RichTable(
        title="[bold cyan]Clients  (yellow=connected | dim=probing)[/bold cyan]",
        expand=True, border_style="blue", show_lines=True
    )
    t.add_column("Status",      style="white",       width=11)
    t.add_column("Client MAC",  style="bold yellow", width=22)
    t.add_column("Vendor",      style="white",       width=14)
    t.add_column("SSID / Network",  style="white",   width=20)
    t.add_column("AP BSSID",    style="dim white",   width=21)
    t.add_column("Frames",      style="cyan",        width=7)
    t.add_column("Last Seen",   style="dim white",   width=10)

    now = time.time()

    # Split into connected and probing
    connected = {
        mac: info for mac, info in client_info.items()
        if info.get("bssid") and info.get("bssid") != "---"
    }
    probing = {
        mac: info for mac, info in client_info.items()
        if not info.get("bssid") or info.get("bssid") == "---"
    }

    # Connected clients first — sorted by activity
    for mac, info in sorted(connected.items(),
        ...
        vendor   = mac_vendors.get(mac, info.get("vendor", "Unknown"))
    ...
    # Probing clients after — dimmed
    for mac, info in sorted(probing.items(),
                            key=lambda x: x[1].get("last_seen", 0),
                            reverse=True)[:8]:
        last     = int(now - info.get("last_seen", now))
        last_str = f"{last}s ago" if last < 60 else f"{last//60}m ago"
        vendor   = mac_vendors.get(mac, "Unknown")
        t.add_row(
            "[dim]PROBING[/dim]",
            f"[dim]{mac}[/dim]",
            f"[dim]{vendor[:14]}[/dim]",
    ...
    if not connected and not probing:
        t.add_row(
            "[dim]--[/dim]",
            "[dim]No clients detected yet[/dim]",
            "", "", "", "", ""
        )
    ...
    with Live(layout, refresh_per_second=2, screen=True):
        while not stop_threads:
            ...
            if tick % 8 < 4:
                ...
                connected_count = sum(1 for info in client_info.values() if info.get("bssid") and info.get("bssid") != "---")
                probing_count   = total_clients - connected_count
                stats.add_row("Clients Connected",    f"[bold green]{connected_count}[/bold green]")
                stats.add_row("Clients Probing",      f"[dim]{probing_count}[/dim]")
...
def main():
    ...
    mode_table = RichTable(border_style="blue", show_lines=True,
                           title="[bold cyan]PWCC v6.0 - Audit Modes[/bold cyan]")
