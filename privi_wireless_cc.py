...
class PWCCReport(FPDF):
    def header(self):
        ...
        self.cell(0, 10, "PriVi Wireless Audit Report", ln=True)
        self.set_xy(10, 20)
        self.set_font("Helvetica", "", 10)
        self.set_text_color(180, 180, 180)
        self.cell(0, 8, f"PriViSecurity  |  Analyst: {AUTHOR}  |  {TOOL} v{VERSION}", ln=True)
        self.ln(18)
...
def generate_pdf_report(scan_data: dict, handshake_count: int,
    ...
    pdf.cell(45, 8, "  BSSID",        fill=True, ln=False)
    pdf.cell(65, 8, "SSID",           fill=True, ln=False)
    pdf.cell(15, 8, "CH",             fill=True, ln=False, align="C")
    pdf.cell(20, 8, "Signal",         fill=True, ln=False, align="C")
    pdf.cell(0,  8, "802.11w (MFP)",  fill=True, ln=True,  align="C")
    ...
    for bssid, info in scan_data.items():
        ...
        pdf.cell(45, 7, f"  {bssid}",    fill=True, ln=False)
        pdf.cell(65, 7, ssid[:30],        fill=True, ln=False)
        pdf.cell(15, 7, ch,               fill=True, ln=False, align="C")
        pdf.cell(20, 7, f"{sig} dBm",     fill=True, ln=False, align="C")
        pdf.cell(0,  7, mfp_display,      fill=True, ln=True,  align="C")
    pdf.ln(4)
    ...
    if evil_twins:
        ...
        pdf.set_font("Helvetica", "B", 9)
        pdf.cell(0, 7, f"  ⚠ EVIL TWIN DETECTED: {', '.join(evil_twins)}", ln=True)
        pdf.set_font("Helvetica", "", 9)
        pdf.set_text_color(40, 40, 40)
        pdf.multi_cell(
            0, 6,
    ...
    if not evil_twins and not unprotected and handshake_count == 0:
        pdf.cell(0, 7, "  No significant security observations during this session.", ln=True)
    pdf.ln(4)
